<script lang="ts">
    ...
    let score: number | undefined = $derived(calculateScore(difficulty, playerCount, victory, remainingInvaderCards, revealedInvaderCards, remainingDahan, blightOnIsland))

    function calculateScore(difficulty: number, playerCount: number, victory: boolean, remainingInvaderCards: number | undefined, revealedInvaderCards: number | undefined, remainingDahan: number | undefined, blightOnIsland: number | undefined): number | undefined {
        let score: number;
...
<div class="flex flex-col gap-2">
    <div class="flex items-center space-x-2">
        <Switch bind:checked={victory} id="victory-toggle" class="data-[state=checked]:bg-green-400 data-[state=unchecked]:bg-red-400">Victory</Switch>
