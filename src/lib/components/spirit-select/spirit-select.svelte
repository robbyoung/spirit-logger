...
<Popover.Root bind:open>
  <Popover.Trigger asChild let:builder>
   <Button
    ...
    role="combobox"
    aria-expanded={open}
    class="w-full h-auto justify-between py-2"
   >
    <div class="flex gap-2 h-[40px]">
      {#if selectedSpirit}
        ...
        <p class="h-[1.25rem] my-auto">{selectedSpirit.name}</p>
      {:else}
      ...
      {#key search}
        ...
        {#each spirits as spirit}
          ...
          <Check
            class={cn(
            "mr-2 h-4 w-4",
            value !== spirit.id && "text-transparent"
            )}
          />
          <Avatar.Root>
          ...
          <p class="h-[1.25rem] ml-2 my-auto">{spirit.name}</p>
          </Command.Item>
        {/each}
        </Command.Group>
      {/key}
