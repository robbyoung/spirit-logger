  ...
  let difficulty = $derived(
    adversaries
      .map((adversary) => {
        ...
        return (
          allAdversaries.find((a) => a.id === adversary.id)?.levels[adversary.level].difficulty ?? 0
        );
      })
  ...
  <Card.Content>
    <form>
      <div class="grid w-full items-center gap-4">
        ...
        <div>
          ...
          <fieldset id="spirits" class="flex flex-col gap-1">
            {#each spirits as _, i}
              <SpiritSelect
                id={`spirit-${i}`}
                bind:value={spirits[i]}
                placeholder={`Player ${i + 1} Spirit`}
              ></SpiritSelect>
            {/each}
        ...
        <div>
          ...
          <fieldset id="adversaries" class="flex flex-col gap-1">
            {#each adversaries as _, i}
              <AdversarySelect
                id={`adversary-${i}`}
                bind:value={adversaries[i].id}
                bind:level={adversaries[i].level}
        ...
        <ScoreCalculator {difficulty} spiritCount={spirits.length} />
